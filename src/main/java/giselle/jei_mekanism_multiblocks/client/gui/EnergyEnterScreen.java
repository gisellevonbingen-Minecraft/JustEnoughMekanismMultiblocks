...
public class EnergyEnterScreen extends Screen
{
	...
	protected void init()
	{
		...
		this.editBox = new EditBox(this.font, this.boxX, this.boxY + this.font.lineHeight, this.boxWidth, 20, this.editBox, TextComponent.EMPTY);
	...
	public void tick()
	{
		...
		if (this.julesChanged)
		{
			...
			this.editBox.setValue(new TranslatableComponent("%s %s", this.displayUnit.convertToAsFloatingLong(this.jules), new TranslatableComponent(this.displayUnit.getTranslationKey())).getString());
	...
	public void render(PoseStack pPoseStack, int mouseX, int mouseY, float partialTick)
	{
		...
		if (this.displayText == null || !this.lastJules.equals(this.jules) || this.lastUnit != this.displayUnit)
		{
			...
			this.displayText = new TranslatableComponent("=> %s", UnitDisplayUtils.getDisplayShort(this.displayUnit.convertToAsFloatingLong(this.jules), this.displayUnit));
		}

		int titleWidth = this.font.width(this.title);
		this.font.drawShadow(pPoseStack, this.title, this.editBox.x + (this.editBox.getWidth() - titleWidth) / 2, this.editBox.y - this.font.lineHeight - 4, 0xFFFFFFFF);

		int textX = this.editBox.x;
		int textY = this.editBox.y + this.editBox.getHeight() + 2;
		this.font.drawShadow(pPoseStack, this.displayText, textX, textY, 0xFFFFFFFF);

		int exampleX = this.boxX;
		int exampleY = this.boxY + this.boxHeight - this.font.lineHeight * 2;
		this.font.drawShadow(pPoseStack, COMPONENT_EXAMPLE, exampleX, exampleY, 0xFFFFFFFF);
		this.font.drawShadow(pPoseStack, COMPONENT_EXAMPLES, exampleX + 5, exampleY + this.font.lineHeight, 0xFFFFFFFF);
	}
