...
public class FissionLayoutScreen extends Screen
{
	...
	private static final ITextComponent COMPONENT_RESET = new TranslationTextComponent("text.jei_mekanism_multiblocks.reset");
	private static final ITextComponent COMPONENT_CLEAR = new TranslationTextComponent("text.jei_mekanism_multiblocks.clear");
	private static final ITextComponent COMPONENT_SAVE = new TranslationTextComponent("text.jei_mekanism_multiblocks.save");
	private static final ITextComponent COMPONENT_CHECK = new StringTextComponent("✔");
	private static final ITextComponent COMPONENT_ERROR = new StringTextComponent("❌");
	private static final ITextComponent COMPONENT_JSON_SYNTAX_ERROR = new TranslationTextComponent("text.jei_mekanism_multiblocks.json_syntax_error");
	private static final ITextComponent COMPONENT_IMPORT = new TranslationTextComponent("text.jei_mekanism_multiblocks.import");
	private static final ITextComponent COMPONENT_IMPORT_FROM_CLIPBOARD = new TranslationTextComponent("text.jei_mekanism_multiblocks.import_from_clipboard");
	private static final ITextComponent COMPONENT_IMPORTED_SUCCESSFULLY = new TranslationTextComponent("text.jei_mekanism_multiblocks.imported_successfully");
	private static final ITextComponent COMPONENT_EXPORT = new TranslationTextComponent("text.jei_mekanism_multiblocks.export");
	private static final ITextComponent COMPONENT_EXPORT_TO_CLIPBOARD = new TranslationTextComponent("text.jei_mekanism_multiblocks.export_to_clipboard");
	private static final ITextComponent COMPONENT_EXPORTED_SUCCESSFULLY = new TranslationTextComponent("text.jei_mekanism_multiblocks.exported_successfully");
	private static final ITextComponent COMPONENT_MAX_BURN_RATE = new TranslationTextComponent("text.jei_mekanism_multiblocks.result.max_burn_rate");
	private static final ITextComponent COMPONENT_BOIL_EFFICIENCY = new TranslationTextComponent("text.jei_mekanism_multiblocks.result.boil_efficiency");
	...
	private final Object2IntMap<ITextComponent> pillarTextWidthCache = new Object2IntOpenHashMap<>();
	...
	public FissionLayoutScreen(ITextComponent title, FissionReactorCategoryWidget widget)
	{
		super(title);
	...
	protected void init()
	{
		...
		this.addButton(this.widthSlider);
		...
		this.addButton(this.lengthSlider);
		...
		this.addButton(this.heightSlider);
		...
		this.addButton(this.burnRateSlider);
		...
		this.addButton(this.resultsList);
	...
	public void render(MatrixStack pMatrixStack, int mouseX, int mouseY, float partialTick)
	{
		...
		for (int z = 0; z < layout.getLength(); z++)
		{
			...
			for (int x = 0; x < layout.getWidth(); x++)
			{
				...
				this.blit(pMatrixStack, coordX, coordZ, u, v, CELL_WIDTH, CELL_HEIGHT);
		...
		if (this.resultDirty)
		{
			...
			this.simulation.createStableTempWidgets(this.resultsList::addChild, this.burnRate);
	...
	public boolean mouseScrolled(double mouseX, double mouseY, double pDelta)
	{
		...
		if (layout.canPillar(x, z))
		{
			...
			int next = pDelta > 0.0D ? (prev + 1) : (prev - 1);
	...
	private void onButtonPress(AbstractButton button)
	{
		...
		else if (button == this.importButton)
		{
			try
			{
				...
				JsonObject jobject = (JsonObject) new JsonParser().parse(clipboard);

				this.widthSlider.getSlider().setValue(JSONUtils.getAsInt(jobject, "width"));
				this.lengthSlider.getSlider().setValue(JSONUtils.getAsInt(jobject, "length"));
				this.heightSlider.getSlider().setValue(JSONUtils.getAsInt(jobject, "height"));

				JsonArray lines = JSONUtils.getAsJsonArray(jobject, "grid", new JsonArray());
				...
				this.updateLayout();
				this.burnRateSlider.getSlider().setValue(JSONUtils.getAsInt(jobject, "burnRate"));
