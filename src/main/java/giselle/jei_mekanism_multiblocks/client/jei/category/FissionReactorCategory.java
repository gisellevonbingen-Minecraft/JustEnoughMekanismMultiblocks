...
public class FissionReactorCategory extends MultiblockCategory<FissionReactorCategory.FissionReactorCategoryWidget>
{
	...
	public static class FissionReactorCategoryWidget extends MultiblockWidget
	{
		...
		public void createStableTempWidget(Consumer<AbstractWidget> consumer, Component with, double conductivity, double thermalEnthalpy)
		{
			...
			ResultWidget tempWidget = new ResultWidget(new TranslatableComponent("text.jei_mekanism_multiblocks.result.temp_with", with), MekanismUtils.getTemperatureDisplay(stableTemp, TemperatureUnit.KELVIN, true));
			...
			if (warning)
			{
				...
				tempWidget.setTooltip(burnRateTooltip, //
						...
						new TranslatableComponent("text.jei_mekanism_multiblocks.tooltip.max_damage_ticks", TextUtils.format(maxDamageTicks)).withStyle(ChatFormatting.RED));
			}
			else
			{
				tempWidget.setTooltip(burnRateTooltip);
