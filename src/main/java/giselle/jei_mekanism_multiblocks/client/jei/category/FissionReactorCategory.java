...
public class FissionReactorCategory extends MultiblockCategory<FissionReactorCategory.FissionReactorCategoryWidget>
{
	...
	public static class FissionReactorCategoryWidget extends MultiblockWidget
	{
		...
		public void createStableTempWidgets(Consumer<Widget> consumer)
		{
		...
		public void createStableTempWidget(Consumer<Widget> consumer, ITextComponent with, Coolant coolant)
		{
			this.createStableTempWidget(consumer, with, coolant.getConductivity(), coolant.getThermalEnthalpy());
		}

		public void createStableTempWidget(Consumer<Widget> consumer, ITextComponent with, double conductivity, double thermalEnthalpy)
		{
			double stableTemp = this.getCoolingStableTemp(conductivity, thermalEnthalpy);
			ResultWidget tempWidget = new ResultWidget(new TranslationTextComponent("text.jei_mekanism_multiblocks.result.temp_with", with), MekanismUtils.getTemperatureDisplay(stableTemp, TemperatureUnit.KELVIN, true));
			...
			if (warning)
			{
				...
				tempWidget.setTooltip(burnRateTooltip, //
						...
						new TranslationTextComponent("text.jei_mekanism_multiblocks.tooltip.max_damage_ticks", TextUtils.format(maxDamageTicks)).withStyle(TextFormatting.RED));
			}
			else
			{
				tempWidget.setTooltip(burnRateTooltip);
		...
		public void setAdvancedLayout(Layout layout)
		{
			...
			this.advancedLayout.setWidth(MathHelper.clamp(this.advancedLayout.getWidth(), this.getDimensionWidthMin(), this.getDimensionWidthMax()));
			this.advancedLayout.setLength(MathHelper.clamp(this.advancedLayout.getLength(), this.getDimensionLengthMin(), this.getDimensionLengthMax()));
			this.advancedLayout.setHeight(MathHelper.clamp(this.advancedLayout.getHeight(), this.getDimensionHeightMin(), this.getDimensionHeightMax()));
		...
		public static class Layout
		{
			...
			public int getPillar(int x, int z)
			{
				...
				{
					return MathHelper.clamp(this.pillars[z][x], 0, this.getPillarMax());
				}
			...
			public void setPillar(int x, int z, int pillar)
			{
				if (this.canPillar(x, z))
				{
					this.pillars[z][x] = MathHelper.clamp(pillar, 0, this.getPillarMax());
				}
			...
			{
				this.width = MathHelper.clamp(width, this.widthMin, this.widthMax);
			}
			...
			{
				this.length = MathHelper.clamp(length, this.lengthMin, this.lengthMax);
			}
			...
			{
				this.height = MathHelper.clamp(height, this.heightMin, this.heightMax);
