...
public class FusionReactorCategory extends MultiblockCategory<FusionReactorCategory.FusionReactorCategoryWidget>
{
	...
	{
		super(helper, RECIPE_TYPE, GeneratorsLang.FUSION_REACTOR.translate(), new ItemStack(GeneratorsBlocks.FUSION_REACTOR_CONTROLLER));
	}
	...
	public static class FusionReactorCategoryWidget extends MultiblockWidget
	{
		...
		{
			double k = active ? MekanismGeneratorsConfig.generators.fusionWaterHeatingRatio.get() : 0.0D;
			double caseAirConductivity = MekanismGeneratorsConfig.generators.fusionCasingThermalConductivity.get();
			double aMin = burnTemperature * burnRatio * plasmaCaseConductivity * (k + caseAirConductivity) / (MekanismGeneratorsConfig.generators.energyPerFusionFuel.get() * burnRatio * (plasmaCaseConductivity + k + caseAirConductivity) - plasmaCaseConductivity * (k + caseAirConductivity));
